...
create table if not exists profiles (
  ...
  recovery_code_hash text,   -- SHA-256 du code de récupération (mot de passe oublié)
...
create policy "objets_insert"   on objets for insert
...
-- Policies bucket : insert pour les connectés, select pour tous
...
create policy "storage_insert" on storage.objects
  for insert with check (bucket_id = 'objets' and auth.role() = 'authenticated');
